...
' | sudo -E tee /etc/nginx/blockuseragents.rules >/dev/null 2>&1
    ...
    sudo mkdir -p /var/yiimp/backup
    sudo mkdir -p /var/yiimp/log
    cd $HOME/yiimp/stratum
...
user=root
password='"${rootpasswd}"'
' | sudo -E tee ~/.my_yiimp.cnf >/dev/null 2>&1
      sudo chmod 0600 ~/.my_yiimp.cnf
...
define('"'"'YAAMP_LOGS'"'"', '"'"'/var/yiimp/log'"'"');
...
output "Whew that was fun, just some reminders. Your mysql information is saved in ~/.my_yiimp.cnf. this installer did not directly install anything required to build coins."
output ""
...
output ""
output "Please donate to 3CsdpkawMuhSqBrdvCcHacSyuzmjzHVTTR to further develop and refine this ez-yiimp installer."
output ""
